...
def load_component(name: str, script: Path):
...
def acquire_workflow_lock_in_child(
    ...
    workflow = load_component("workflow_lock_child", Path(script))
...
def run_component(script: Path, *args: str) -> str:
...
def test_workflow_runtime_resumes_from_journal(tmp_path: Path) -> None:
    ...
    first = run_component(script, "demo")
    resumed = run_component(script, "resume")
