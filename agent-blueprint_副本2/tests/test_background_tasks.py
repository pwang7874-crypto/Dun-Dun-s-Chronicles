...
COMPONENT = ROOT / "s11_background_tasks" / "code.py"
...
def test_s11_keeps_the_s04_kernel_and_adds_one_bash_option():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        bash = next(tool for tool in component.TOOLS if tool["name"] == "bash")
...
def test_background_execution_requires_an_explicit_bash_flag():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert not component.should_run_background("bash", {"command": "npm install"})
        assert component.should_run_background(
            "bash", {"command": "printf ready", "run_in_background": True}
        )
        assert not component.should_run_background(
...
def test_background_bash_passes_permission_before_dispatch():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        component.client.messages.create = lambda **_: responses.pop(0)
        history = [{"role": "user", "content": "Delete the directory"}]

        component.agent_loop(history)

        assert not component.background_tasks
...
def test_completed_result_is_collected_once_before_a_later_llm_call():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        task_id = component.start_background_task(block)
        assert wait_until(
            lambda: component.background_tasks[task_id]["status"] == "completed"
        )
        ...
        assert component.collect_background_results() == []
