...
COMPONENT = ROOT / "s06_subagent" / "code.py"


def load_component(temp_cwd: Path):
    ...
    spec = importlib.util.spec_from_file_location("s06_subagent_test", COMPONENT)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Unable to load {COMPONENT}")
...
def test_s06_is_kernel_plus_task():
    ...
    base_names = {tool["name"] for tool in component.BASE_TOOLS}
    parent_names = {tool["name"] for tool in component.TOOLS}
    child_names = {tool["name"] for tool in component.SUB_TOOLS}
    ...
    assert component.TASK_TOOL["input_schema"]["required"] == ["prompt"]
    assert component.large_output_hook in component.HOOKS["PostToolUse"]
...
def test_subagent_starts_with_fresh_messages_and_returns_final_text():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        result = component.run_subagent("Read note.txt and report its contents.")
...
def test_subagent_file_tools_keep_the_kernel_permission_boundary():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch.object(builtins, "input", return_value="n"):
            result = component.execute_tool(block, component.SUB_HANDLERS)
